 ...
 ******************************************************************************/
 ...
 QueueHandle_t xQueueTempBuffer = NULL;  ///< Queue for temperature samples. 
 QueueHandle_t xQueueIntensityBuffer = NULL; ///< Queue to send the light intensity to the cloud
 ...
 volatile bool g_heating_enable = false;  // Initialize heating flag to false (heating is off by default)
 volatile bool g_requestIntensity = false;
 ...
 void SubscribeHandlerHeatingTopic(MessageData *msgData) {
    ...
    if (payload_len == 5 && strncmp(payload, "start", 5) == 0) {
        ...
        // Optionally log or debug print that heating was enabled
...
/***************************************************************************//**
 * @brief  MQTT callback for the *detect* command.
 *
...
/**
 * @brief Publish one queued temperature sample.
 *
 * Pops a float from @ref xQueueTempBuffer, scales it to centi-degrees
 * (°C × 100), formats as text, and sends it to
...
/***************************************************************************//**
 * @brief Publish one queued light-intensity sample via MQTT.
 *
 * Removes the oldest sample from @ref xQueueIntensityBuffer, converts the
 * integer (lux × 100) to ASCII, and publishes it to
...
/**
 * @brief  Publish a notice that the MCU is about to reset.
 *
 * The function sends a short, non-retained message (`"mcu_reset"`) with
 * **QoS 1** on #RESET_MQTT_TOPIC so that external dashboards (e.g. Node-RED)
 * can log or react to the imminent reboot.
 *
...
/***************************************************************************//**
 * @brief  Publish the running firmware version via MQTT.
 *
 * Publishes the null-terminated string in @ref FIRMWARE_VERSION on the topic
 * @ref FIRMWARE_MQTT_TOPIC with **QoS 1** and the *retain* flag set so that
 * dashboards (e.g. Node-RED) always receive the last known version.
 *
...
/*******************************************************************************
 * @brief  Starts the firmware update process via WiFi.
 *
 * This function deletes the previous "application.bin" file (if it exists),
 * and then triggers the WiFi handler to start downloading a new firmware image.
...
/*******************************************************************************
 * @brief  Creates a "Golden Image" backup of the application firmware.
 *
 * This function copies "application.bin" to "g_application.bin" on the filesystem.
 * It first attempts to delete any existing "g_application.bin" file before copying.
