...
/******************************************************************************
 * Added for VEML7700: I2cWriteReadNoStopBlocking_VEML
 ******************************************************************************/
/**
 * @brief  Specialized function for VEML7700: Write (no stop) + Read (Repeated START), blocking.
 * 
 * @param[in] addr        7-bit I2C address of VEML7700 (e.g. 0x10)
 * @param[in] wrBuf       Pointer to the buffer containing register address to write
 * @param[in] wrLen       Number of bytes to write (usually 1 for VEML7700 register address)
 * @param[out] rdBuf      Pointer to the buffer to store read data
 * @param[in] rdLen       Number of bytes to read (usually 2 for VEML7700 data)
 * @return int32_t        ERROR_NONE on success, or error code on failure
 *
 * @note This function uses blocking calls: i2c_master_write_packet_wait_no_stop()
 *       and i2c_master_read_packet_wait(). It also uses a mutex to protect the bus.
 *       It does NOT rely on the FreeRTOS semaphore/callback approach used by
